...
UBYTE Dificultad; //0 = normal mode(CPU solo mueve tras golpear jugador) - 1 = hard mode (CPU mueve siempre)
	...
	while(1) {
		...
		if((score_p1 == 20 || score_p2 == 20) || controller == 1){ //fin del juego.
			if((score_counter_p2 == 20 || score_counter_p1 == 20)){	
				HIDE_SPRITES;
				BGP_REG = 0x27U;
				if (score_counter_p1 == 20 || score_p1 == 20){
				if (score_p2 == 0) score+= 300;
				gotoxy(0, 0);
				printf(" \n");
				printf(" \n");
				printf(" \n");
				...
				if (score_counter_p2 == 20 || score_p2 == 20){
				gotoxy(0, 0);
				printf(" \n");
				printf(" \n");
				printf(" \n");
		...
		if(counter & J_UP && y1>=23){
			y1-=2;
			y2-=2;
			y3-=2;
			if(counter & J_A){ //si presionamos A nuestro paddle va más rápido.
				y1-=2;
				y2-=2;
				y3-=2;
			}
			if(counter & J_B){ //si presionamos B nuestro paddle va más lento.
				y1+=1;
				y2+=1;
				y3+=1;
			}
		...
		if(y5 > ball_pos_y && y4 >= 23 && golpea == 0){
			y4-=2;
			y5-=2;
			y6-=2;
		}
...
//counter & J_START && 
		...
		if (col1 < 16){
			if (Dificultad == 0) golpea=1;//golpea el CPU así que NO se mueve.
			else golpea =0; //si la dificultad es 1 (hard) si se mueve.
			while(temp1!=0 || temp2!=0){
				if(temp1 > 0){
					temp1--;
					ball_pos_x--;
		...
		if(ball_pos_x<23){
			...
			incremento = 0;
			golpea = 0;
			if (score>=37) score -= 37; //restamos puntos al total.
			//CPU anota un tanto.
			controller = 1;
		}
		// pared derecha.
		if(ball_pos_x>145){
			...
			incremento=0;
			golpea = 0;
			score += 74; //añadimos puntos.
			//el jugador  anota un tanto.
			controller = 1;
		}
